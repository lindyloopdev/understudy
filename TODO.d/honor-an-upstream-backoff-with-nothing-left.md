# Honor an upstream's backoff when nothing else is left

**Tag:** understudy / bug

**Design:** [DESIGN.md §Recovery probing](../DESIGN.md#recovery-probing) — an
upstream's `readmitAt` supersedes the schedule, and a target carrying one is never
...
resolve, whatever its health — including one benched until a moment the upstream named that
...
schedule-held fallback it means to rather than the upstream's bench.
