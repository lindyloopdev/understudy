...
as when the streak began. A target demoted at once by an access refusal or an
advertised `429` therefore tells an operator it started failing a full threshold
before its first request ran. Measured: a target whose only failure is at `00:00:00`
is logged `failing_since 1999-12-31T23:59:45Z`.

A stall says nothing at all: `recordStalled` emits its own record and omits the field,
which is the other half of the same disagreement — see
[[log-a-transition-where-it-happens]], where the two emission sites are reconciled.
